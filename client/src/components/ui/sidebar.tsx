...
import { FiMenu } from "react-icons/fi" // ✅ Ícone trocado
  ...
  (
    ...
    const setOpen = React.useCallback(
      (value: boolean | ((value: boolean) => boolean)) => {
        const newOpenValue = typeof value === "function" ? value(open) : value;
        if (setOpenProp) {
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      {...props}
    >
      <FiMenu /> {/* ✅ Ícone trocado */}
      <span className="sr-only">{t('sidebar.toggle')}</span> {/* ✅ Texto traduzido */}
    </Button>
  )
})
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ref={ref}
      data-sidebar="rail"
      aria-label={t('sidebar.toggle')} // ✅ Texto traduzido
      tabIndex={-1}
      onClick={toggleSidebar}
      title={t('sidebar.toggle')} // ✅ Texto traduzido
      className={cn(
...
// ... (Restante dos sub-componentes que não precisam de alteração)
...
const SidebarMenuBadge = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(({ className, ...props }, ref) => (
    <div ref={ref} data-sidebar="menu-badge" className={cn("absolute right-1 flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-xs font-medium tabular-nums text-sidebar-foreground select-none pointer-events-none", "peer-hover/menu-button:text-sidebar-accent-foreground peer-data-[active=true]/menu-button:text-sidebar-accent-foreground", "peer-data-[size=sm]/menu-button:top-1", "peer-data-[size=default]/menu-button:top-1.5", "peer-data-[size=lg]/menu-button:top-2.5", "group-data-[collapsible=icon]:hidden", className)} {...props} />
));
